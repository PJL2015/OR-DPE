	...
	if(check_regression_model(model_))
	{
		...
		else
		{
			for(int k=0; k<sub_prob.l; k++)
				{
					sub_prob.x[k] = prob->x[perm[k]];
					sub_prob.y[k] = (double)(prob->y[perm[k]]-1)/(nr_class -1);
				}		
		}				
...
static const char *solver_type_table[]=
{
	...
	"L2R_SVOR"," ","L2R_SVMOP",//My modification
	...
	double *best_acc_rate, double *best_mae_rate, const char *input_file_name)
{
	...
	*best_mae_rate = INF;
	...
	{
		sprintf(file_name,"%s_%s%g_%g_cvout.log",solver_type_table[param->solver_type],input_file_name,prob->bias,param1.C);
		FILE * cvout = fopen(file_name,"w");
		start=clock();
		...
		for(i=0; i<nr_fold; i++)
		{
			...
			int total_w_size;
			if((submodel->nr_class == 2 ||submodel->param.solver_type == L2R_SVOR||check_regression_model(submodel)))
...
double predict_values(const struct model *model_, const struct feature_node *x, double *dec_values)
{
	...
	int i;
	int nr_w;
	if((nr_class==2 && model_->param.solver_type != MCSVM_CS )||model_->param.solver_type == L2R_SVOR||check_regression_model(model_))
		nr_w = 1;
	else if(model_->param.solver_type == L2R_SVMOP)
		nr_w = nr_class-1;
	else
	...
	if(nr_class==2 && model_->param.solver_type!= L2R_SVOR 
...
static inline double get_w_value(const struct model *model_, int idx, int label_idx) 
{
	...
	else 
	{
		if(label_idx < 0 || label_idx >= nr_class)
			return 0;
		if((nr_class==2 && solver_type != MCSVM_CS )||solver_type ==L2R_SVOR) //add
